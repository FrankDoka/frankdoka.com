...
import diagram from '../../projects/portfolio-site/portfolio-site.svg'
...
<Figure src={diagram} alt="Portfolio site architecture: an MDX content pipeline validated by Zod feeds a Next.js 15 app with Pagefind search, deployed as a static export to Cloudflare Pages." caption="The Next.js 15 + MDX architecture this series builds." />
...
Scroll-triggered fade-ins and staggered card reveals run on a small `IntersectionObserver` hook plus a CSS transition — no animation library, so nothing extra ships to the browser. A component fades and slides into place the first time it scrolls into view.
